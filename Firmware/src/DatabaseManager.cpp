...
void WIFI_CONNECTION::createHandlers()
{
    ...
    server.on("/updateConfigParms", HTTP_GET, [this](AsyncWebServerRequest *request){
        ...
        json.reserve(96);
        json += "{\"TargetHumLow\":";
        json += db_ref.PARMS.getSoilHumTargetLOW();
        json += ",\"TargetHumHigh\":";
        json += db_ref.PARMS.getSoilHumTargetHIGH();
        json += ",\"AlarmHours\":";
        json += alarmParms[0];
        json += ",\"AlarmMinutes\":";
        json += alarmParms[1];
    ...
    server.on("/control/target", HTTP_POST,
        ...
        [this](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total){
            int low;
            int high;
            sscanf((char*)data, "{\"soilTargetLow\":%d,\"soilTargetHigh\":%d}", &low, &high);
            db_ref.PARMS.setSoilHumTarget(low, high);  
        }
    );
...
//====================================SystemParameters=========================================
uint8_t SystemParameters::getSoilHumTargetLOW() { return SoilHumTargetLOW; }
uint8_t SystemParameters::getSoilHumTargetHIGH() { return SoilHumTargetHIGH; }
void SystemParameters::setSoilHumTarget(uint8_t low, uint8_t high) { SoilHumTargetLOW = low; SoilHumTargetHIGH = high; }
